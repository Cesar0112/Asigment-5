...
class Packet:
    ...
    def __str__(self):
        return ('Packet(seqnum=%s, acknum=%s, checksum=%s, payload=%s)'
...
class EntityB:
    ...
    def input(self, packet):
        if (packet.seqnum != self.expecting_bit
            ...
            p = Packet(0, 1-self.expecting_bit, 0, packet.payload)
            pkt_insert_checksum(p)
            to_layer3(self, p)
        else:
            to_layer5(self, Message(packet.payload))
            p = Packet(0, self.expecting_bit, 0, packet.payload)
            pkt_insert_checksum(p)
            to_layer3(self, p)
...
class Simulator:
    ...
    def run(self):
        ...
        while (self.event_list
            ...
            if ev.ev_type == EventType.FROM_LAYER5:
                self._generate_next_arrival()
                j = self.n_sim % 26
                m = bytes([97+j for i in range(Message.MSG_SIZE)])
                if self.trace>2:
                    print(f'          MAINLOOP: data given to student: {m}')
                self.n_sim += 1
                ev.ev_entity.output(Message(m))
  ...
  Call ignored.''')
    ...
    def _valid_message(self, m, method_name):
        if (type(m) is Message
            ...
            and len(m.data) == Message.MSG_SIZE):
            return True
  ...
  Call ignored.''')
    ...
    def _valid_packet(self, p, method_name):
        if (type(p) is Packet
            ...
            and len(p.payload) == Message.MSG_SIZE):
            return True
  ...
  Call ignored.''')
    ...
    def to_layer3(self, entity, packet):
        ...
        p = Packet(seqnum, acknum, checksum, payload)
